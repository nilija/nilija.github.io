...
$userFile = __DIR__ . '/work/users.file'; // Sadrži listu registrovanih korisnika
echo $logFile;
echo $userFile;
...
function isUserRegistered($email, $userFile) {
    ...
    return in_array(trim($email), $users); // Provera da li je e-mail u fajlu
}
...
if (isset($_POST['login'])) {
    ...
    if (!empty($email)) {
        if (isUserRegistered($email, $userFile)) {
            ...
            // Redirekcija na komplet.php sa e-mail argumentom
            header("Location: komplet.php?email=" . urlencode($email));
            exit();
        } else {
...
if (isset($_POST['register'])) {
    ...
    if (!empty($email) && !empty($name) && !empty($surname) && !empty($city) && !empty($userType)) {
        ...
        // Kreiranje foldera za korisnika u work/ direktorijumu
        $userWorkDir = __DIR__ . '/work/' . $email;
...
<div class="grid-container">
    ...
    <!-- Forma za prijavu ili registraciju -->
    <div class="section">
        <h2><?php echo $showRegisterForm ? "Registracija" : "Prijava"; ?></h2>
        <form method="POST">
